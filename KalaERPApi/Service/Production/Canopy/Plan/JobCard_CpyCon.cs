        ...
        public string Submit(JobCard_CpyRequest job_Cpyreq)
        {
            ...
            try
            {
                ...
                { // Main For Loop Start Bracket 
                    ...
                    string TxtCpBoxCode = ComCon.getName("select partcode from BomDetails  where   BomCode='" + Dts[10].ToString().Trim() + "'  and  KitCode Like '003%' and  PartCode LIKE '004%'", "tblCpy1", "partcode");
                    ...
                    cmd.Dispose();
                    //Added RB dis FS    16/07/2025
                    string PrcNo = "";
                    //dsTurretKitForPrc = ComCon.procTranDS(
                    //    "SELECT COUNT(TKITID) AS TKITID FROM TurretKitForPrc WHERE cpcode = '" + StrDispCode_CPYPlan.Trim() +
                    ...
                    //Added RB dis KB    19/09/2025

                    dsTurretKitForPrc = ComCon.procTranDS("SELECT COUNT(TKITID) AS TKITID  FROM TurretKitForPrcDts WHERE TKITID IN (" +
                    ...
                    // Always returns 1 row with the count
                    ...
                    {
                        dsTurretKitGKForPrc = ComCon.procTranDS("SELECT P.rating, B.BOMCode, B.PartCode " +
                                                                  "FROM BOM B " +
                        ...
                        bool hasGKRating =
                            dsTurretKitGKForPrc.Tables["tbl_BOMPartRating"].Rows.Count > 0;
                        // Check for null or DBNull
                        if (Dts[0] != null && !Convert.IsDBNull(Dts[0]))
                        {
                            double.TryParse(Dts[0].ToString().Trim(), out kva);
