            ...
            try
            {
                ...
                else if (DGStageReq.StageNo == 1)
                {
                    ...
                    if (DGStageReq.EngPartCode.Trim().Substring(0, 3) == "001")
                    {
                        ...
                        if (DGStageReq.PrcStatus == "Accepted(OK)")
                        {
                            sb.Append("Stage1Status='D',stage1Date =GetDate() ");
                        }
                        else if (DGStageReq.PrcStatus == "Rework" || DGStageReq.PrcStatus == "Rejection")
                        {
                            sb.Append("Stage1Status='R' ");
                    ...
                    {
                        sb.Remove(0, sb.Length);
                        sb.Append("Update JobCard set Stage1Status='D' where  JobCode='" + DGStageReq.JBCode.Trim() + "'  ");
                        cmd = new SqlCommand(sb.ToString(), con);
                        cmd.Transaction = tran;
                        cmd.ExecuteNonQuery();
                        cmd.Dispose();
        ...
        public string SubmitStage4(DGStage4Request Stage4Req)
        {
            ...
            if (Stage4Req.Remark == "Start")
            {
                ...
                try
                {
                    ...
                    sb.Append("INSERT INTO StockWIP(FromProfitCenterCode,PartCode,ReceivedCode,ReceivedDate,ReceivedQty,ToProfitCenterCode,StockType,PanelTypeId,StageName)");
                    sb.Append(" values('" + Stage4Req.PCCode.Trim() + "','" + Stage4Req.ProductCode.Trim() + "',");
                    sb.Append("'" + PrcNo.Trim() + "',GetDate(),'1','" + Stage4Req.PCCode.Trim() + "',0,'" + CPType[1].Trim() + "','0')");
                    cmd = new SqlCommand(sb.ToString(), con);
                    cmd.Transaction = tran;
                    cmd.ExecuteNonQuery();
                    cmd.Dispose();
...
*/
    ...
    public class DGStageCon
    {
        ...
        CommonCon ComCon = new CommonCon();
        string strSql = "", strKanBan = "";
        public SqlTransaction tran = null;
        #endregion

        public object SqlDbTypeChar { get; private set; }

        public DataTable GetScanDts(string strSrNo, string strPartCode, string strCat, string strStage,string PCCode)
        {
            SqlDataAdapter dAd = new SqlDataAdapter("GetStageScanDts", con);          
            dAd.SelectCommand.CommandType = CommandType.StoredProcedure;
            dAd.SelectCommand.Parameters.Add("@strSrNo", SqlDbType.Char).Value = strSrNo;
        ...
        public string Submit([FromBody] DGStageRequest DGStageReq)
        {
            ...
            try
            {
                ...
                if (DGStageReq.StageNo == 0)//S1 Start
                {
                    sb.Remove(0, sb.Length);
                    sb.Append("INSERT INTO StockWIP(FromProfitCenterCode,PartCode,ReceivedCode,ReceivedDate,ReceivedQty,ToProfitCenterCode,StockType,PanelTypeId,StageName)");
                    sb.Append(" values('" + DGStageReq.PCCode + "','" + DGStageReq.ProductCode.Trim() + "',");
                    sb.Append("'" + DGStageReq.JBCode + "-->" + DGStageReq.EngSrNo + "',GetDate(),'1','" + DGStageReq.PCCode + "',0,'0','StageI')");
                    cmd = new SqlCommand(sb.ToString(), con);
                    cmd.Transaction = tran;
                    cmd.ExecuteNonQuery();
                    cmd.Dispose();
                    ...
                    if (DGStageReq.EngPartCode.Trim().Substring(0, 3) == "001")
                    {
                        ...
                        sb.Remove(0, sb.Length);
                        sb.Append("INSERT INTO StockWIP(FromProfitCenterCode,PartCode,IssueCode,IssueDate,IssueQty,ToProfitCenterCode,StockType,PanelTypeId,StageName)");
                        sb.Append(" values('" + DGStageReq.PCCode + "','" + DGStageReq.EngPartCode.Trim() + "',");
                        sb.Append("'" + DGStageReq.JBCode + "-->" + DGStageReq.EngSrNo + "',GetDate(),'1','" + DGStageReq.PCCode + "',0,'0','StageI')");
                        cmd = new SqlCommand(sb.ToString(), con);
                        cmd.Transaction = tran;
                        cmd.ExecuteNonQuery();
                    ...
                    if (DGStageReq.AltPartcode.Trim().Substring(0, 3) == "002")
                    {
                        ...
                        sb.Remove(0, sb.Length);
                        sb.Append("INSERT INTO StockWIP(FromProfitCenterCode,PartCode,IssueCode,IssueDate,IssueQty,ToProfitCenterCode,StockType,PanelTypeId,StageName)");
                        sb.Append(" values('" + DGStageReq.PCCode + "','" + DGStageReq.AltPartcode.Trim() + "',");
                        sb.Append("'" + DGStageReq.JBCode + "-->" + DGStageReq.EngSrNo + "',GetDate(),'1','" + DGStageReq.PCCode + "',0,'0','StageI')");
                        cmd = new SqlCommand(sb.ToString(), con);
                        cmd.Transaction = tran;
                        cmd.ExecuteNonQuery();
                    ...
                    ChkDGRate = "0";
                    ChkDGRate = ComCon.getTranName("select isnull(Count(Partcode),0) as PartCnt from PCStageWiseRate where PCCode='" + DGStageReq.PCCode + "' and Partcode='" + DGStageReq.ProductCode + "'  and StageName='StageI'  ", "TblPCStageWiseRate", "PartCnt", con, tran);
                    if (ChkDGRate == "0")
                    {
                        GetDGRate = ComCon.getTranName("select isnull(Sum(SuppRate),0) as Rate from BOMDetails where Kitcode='" + DGStageReq.ProductCode + "'  and SubString(Partcode,1,3) in ('001','002')  ", "TblStageWiseRate", "Rate", con, tran);

                        sb.Remove(0, sb.Length);
                        sb.Append("INSERT INTO PCStageWiseRate(Dt,PCCode,PartCode,StageName,JobCode,Rate)");
                        sb.Append(" values(GetDate(),'" + DGStageReq.PCCode + "','" + DGStageReq.ProductCode.Trim() + "',");
                        sb.Append("'StageI','" + DGStageReq.JBCode.Trim() + "','" + double.Parse(GetDGRate.Trim()) + "')");
                        cmd = new SqlCommand(sb.ToString(), con);
                        cmd.Transaction = tran;
                        cmd.ExecuteNonQuery();
                        cmd.Dispose();
                        ...
                        sb.Remove(0, sb.Length);
                        sb.Append("INSERT INTO PCStageWiseRateChange(Dt,PCCode,PartCode,StageName,JobCode,Rate)");
                        sb.Append(" values(GetDate(),'" + DGStageReq.PCCode + "','" + DGStageReq.ProductCode.Trim() + "',");
                        sb.Append("'StageI','" + DGStageReq.JBCode.Trim() + "','" + double.Parse(GetDGRate.Trim()) + "')");
                        cmd = new SqlCommand(sb.ToString(), con);
                        cmd.Transaction = tran;
                    ...
                    else
                    {
                        ...
                        sb.Remove(0, sb.Length);
                        sb.Append("Update PCStageWiseRate set Rate='" + double.Parse(GetDGRate.Trim()) + "',Dt=GetDate(),JobCode='" + DGStageReq.JBCode.Trim() + "' ");
                        sb.Append(" Where PCCode='" + DGStageReq.PCCode + "' and Partcode='" + DGStageReq.ProductCode.Trim() + "' and StageName='StageI' ");
                        cmd = new SqlCommand(sb.ToString(), con);
                        cmd.Transaction = tran;
                        cmd.ExecuteNonQuery();
                        cmd.Dispose();
                ...
                else if (DGStageReq.StageNo == 1) //S1 End
                {
                    ...
                    sb.Append("INSERT INTO StockWIP(FromProfitCenterCode,PartCode,ReceivedCode,ReceivedDate,ReceivedQty,ToProfitCenterCode,StockType,PanelTypeId,StageName)");
                    sb.Append(" values('" + DGStageReq.PCCode + "','" + DGStageReq.ProductCode.Trim() + "',");
                    sb.Append("'" + DGStageReq.JBCode + "-->" + DGStageReq.EngSrNo + "',GetDate(),'1','" + DGStageReq.PCCode + "',0,'0','StageIII')");
                    cmd = new SqlCommand(sb.ToString(), con);
                    cmd.Transaction = tran;
                    cmd.ExecuteNonQuery();
                    cmd.Dispose();
                ...
                else if (DGStageReq.StageNo == 3) //S2(Now S2)
                {
                    ...
                    sb.Append("INSERT INTO StockWIP(FromProfitCenterCode,PartCode,ReceivedCode,ReceivedDate,ReceivedQty,ToProfitCenterCode,StockType,PanelTypeId,StageName)");
                    sb.Append(" values('" + DGStageReq.PCCode + "','" + DGStageReq.ProductCode.Trim() + "',");
                    sb.Append("'" + DGStageReq.JBCode + "-->" + DGStageReq.EngSrNo + "',GetDate(),'1','" + DGStageReq.PCCode + "',0,'0','StageIV')");
                    cmd = new SqlCommand(sb.ToString(), con);
                    cmd.Transaction = tran;
                    cmd.ExecuteNonQuery();
                    cmd.Dispose();
                    ...
                    if (DGStageReq.CpyPartcode.Trim().Substring(0, 2) == "40")
                    {
                        ...
                        sb.Remove(0, sb.Length);
                        sb.Append("INSERT INTO StockWIP(FromProfitCenterCode,PartCode,IssueCode,IssueDate,IssueQty,ToProfitCenterCode,StockType,PanelTypeId,StageName)");
                        sb.Append(" values('" + DGStageReq.PCCode + "','" + DGStageReq.CpyPartcode.Trim() + "',");
                        sb.Append("'" + DGStageReq.JBCode + "-->" + DGStageReq.EngSrNo + "',GetDate(),'1','" + DGStageReq.PCCode + "',0,'0','StageIII')");
                        cmd = new SqlCommand(sb.ToString(), con);
                        cmd.Transaction = tran;
                        cmd.ExecuteNonQuery();
                    ...
                    if (DGStageReq.BatPartcode.Trim().Length > 2)
                    {
                        if (DGStageReq.BatPartcode.Trim().Substring(0, 3) == "010")
                        {
                            ...
                            sb.Remove(0, sb.Length);
                            sb.Append("INSERT INTO StockWIP(FromProfitCenterCode,PartCode,IssueCode,IssueDate,IssueQty,ToProfitCenterCode,StockType,PanelTypeId,StageName)");
                            sb.Append(" values('" + DGStageReq.PCCode + "','" + DGStageReq.BatPartcode.Trim() + "',");
                            sb.Append("'" + DGStageReq.JBCode + "-->" + DGStageReq.EngSrNo + "',GetDate(),'1','" + DGStageReq.PCCode + "',0,'0','StageIII')");
                            cmd = new SqlCommand(sb.ToString(), con);
                            cmd.Transaction = tran;
                            cmd.ExecuteNonQuery();
                    ...
                    if (double.Parse(strkVA) > 160)
                    {
                        if (DGStageReq.Bat2Partcode.Trim().Length > 2)
                        {
                            if (DGStageReq.Bat2Partcode.Trim().Substring(0, 3) == "010")
                            {
                                ...
                                sb.Remove(0, sb.Length);
                                sb.Append("Update StockWIP set IssueQty=IssueQty+1 where FromProfitCenterCode='" + DGStageReq.PCCode + "' and Issuecode='" + DGStageReq.JBCode + "-->" + DGStageReq.EngSrNo + "' and StageName='StageIII' ");
                                sb.Append(" and Partcode='" + DGStageReq.BatPartcode.Trim() + "' and ToProfitCenterCode='" + DGStageReq.PCCode + "' ");
                                cmd = new SqlCommand(sb.ToString(), con);
                                cmd.Transaction = tran;
                                cmd.ExecuteNonQuery();
                                cmd.Dispose();
                    ...
                    ChkDGRate = "0";
                    ChkDGRate = ComCon.getTranName("select isnull(Count(Partcode),0) as PartCnt from PCStageWiseRate where PCCode='" + DGStageReq.PCCode + "' and Partcode='" + DGStageReq.ProductCode + "'  and StageName='StageIV'  ", "TblPCStageWiseRate", "PartCnt", con, tran);
                    if (ChkDGRate == "0")
                    {

                        GetDGRate = ComCon.getTranName(" Select isNull(Round(Sum(Rate),0),0) as Rate From ( select isnull(Sum(Qty * SuppRate), 0) as Rate from BOMDetails " +
                        " where Kitcode = '" + DGStageReq.ProductCode + "'  and SubString(Partcode, 1, 3) in ('001', '002', '010') " +
                       "  Union All " +
                        " select Top 1 Rate from ProfitCenterplDetailsChanged PC Inner Join " +
                        " (select BOMCode, Partcode  from BOMDetails where Kitcode = '" + DGStageReq.ProductCode + "'  and SubString(Partcode, 1, 3) in ('401')) as B " +
                       "  On PC.PartCode = B.Partcode and PC.BomCode = B.BOMCode " +
                       "  where ProfitCenterCode = '01.005'and PC.Partcode = '" + DGStageReq.CpyPartcode + "') as T ", "TblStageWiseRate", "Rate", con, tran);

                        sb.Remove(0, sb.Length);
                        sb.Append("INSERT INTO PCStageWiseRate(Dt,PCCode,PartCode,StageName,JobCode,Rate)");
                        sb.Append(" values(GetDate(),'" + DGStageReq.PCCode + "','" + DGStageReq.ProductCode.Trim() + "',");
                        sb.Append("'StageIV','" + DGStageReq.JBCode.Trim() + "','" + double.Parse(GetDGRate.Trim()) + "')");
                        cmd = new SqlCommand(sb.ToString(), con);
                        cmd.Transaction = tran;
                        cmd.ExecuteNonQuery();
                        cmd.Dispose();
                    ...
                    else
                    {
                      ...
                      "  where ProfitCenterCode = '01.005'and PC.Partcode = '" + DGStageReq.CpyPartcode + "') as T ", "TblStageWiseRate", "Rate", con, tran);
                        ...
                        sb.Remove(0, sb.Length);
                        sb.Append("Update PCStageWiseRate set Rate='" + double.Parse(GetDGRate.Trim()) + "',Dt=GetDate(),JobCode='" + DGStageReq.JBCode.Trim() + "' ");
                        sb.Append(" Where PCCode='" + DGStageReq.PCCode + "' and Partcode='" + DGStageReq.ProductCode.Trim() + "' and StageName='StageIV' ");
                        cmd = new SqlCommand(sb.ToString(), con);
                        cmd.Transaction = tran;
                        cmd.ExecuteNonQuery();
                        cmd.Dispose();
        ...
        */
        ...
        public DataTable GetPodprcDts(string strPrdPartCode, string strPCCode)
        {
            ...
            if (StrBOMCode.Trim() != "0")
            {
                ...
                else if (strPCCode.ToString().Trim() == "03.051")
                {
                    strSql = " select BD.PartCode,partdesc+' [' +BD.partcode +'] ' as PartDesc,'' as SerialNo,u.UName,Qty,BD.Mob,P.Kit,p.conversionvalue,p.UomCode,'0' as StockQty,'0' as TotalQty," +
                             " SuppRate as Rate, QAP, Amount, P.CategoryID " +
                             " from #TempBOMdetails BD Inner Join Part P On Bd.Partcode=P.partcode Inner join UOM U On P.UOMCode=U.Uid " +
                             " where bd.KitCode='" + strPrdPartCode.Trim() + "' and bd.BOMCode='" + ProdDts[0].Trim() + "' and substring(BD.PartCode,1,3) Not in ('001','002','010','401','003')   " +
                             //" and BD.PartCode Not in ('0000201251202000000','0000201251201500000') " +
                             " Union All " +
                             " select BD.PartCode,partdesc + ' [' + BD.partcode + '] ' as PartDesc,'' as SerialNo,u.UName,Qty,BD.Mob,P.Kit,p.conversionvalue,p.UomCode,'0' as StockQty,'0' as TotalQty," +
                             " SuppRate as Rate, QAP, Amount,  P.CategoryID " +
                             " from #TempBOMdetails BD Inner Join Part P On Bd.Partcode = P.partcode Inner join UOM U On P.UOMCode = U.Uid " +
                             " where bd.BOMCode = '" + ProdDts[0].Trim() + "' " +
                             " and (Bd.partcode in ('0000207040100000','0000201363100010000') )  " +
                             " Union All " +
                             " select BD.PartCode,partdesc + ' [' + BD.partcode + '] ' as PartDesc,'' as SerialNo,u.UName,Qty,BD.Mob,P.Kit,p.conversionvalue,p.UomCode,'0' as StockQty,'0' as TotalQty, " +
                             " SuppRate as Rate,0 as QAP, 0 as Amount,  P.CategoryID " +
                             " from BOMdetails BD Inner Join Part P On Bd.Partcode = P.partcode Inner join UOM U On P.UOMCode = U.Uid " +
                             " where bd.BOMCode ='" + ProdDts[0].Trim() + "'  " +
                             " and (BD.PartCode in (select Partcode from PartTOCDetailsSupplier where Product = 'KB' and ForPCCode = '03.051'))   " +
                             " and bd.KitCode = '" + strPrdPartCode.Trim() + "' " +
                             //Added For DG kit KANBAN
                             " Union All " +
                             " select BD.PartCode,partdesc + ' [' + BD.partcode + '] ' as PartDesc,'' as SerialNo,u.UName,Qty,BD.Mob,P.Kit,p.conversionvalue,p.UomCode,'0' as StockQty,'0' as TotalQty, " +
                             " SuppRate as Rate,0 as QAP, 0 as Amount,  P.CategoryID " +
                             " from BOMdetails BD Inner Join Part P On Bd.Partcode = P.partcode Inner join UOM U On P.UOMCode = U.Uid " +
                             " where bd.BOMCode ='" + ProdDts[0].Trim() + "' " +
                             " and(BD.PartCode in (select Partcode from PartTOCDetailsSupplier where Product = 'KB' and ForPCCode = '03.051' and Active='1'))   " +
                             " and bd.KitCode in (select Partcode From BOMDetails where BOMCode = '" + ProdDts[0].Trim() + "' and Partcode like '005%' and substring(partcode,10,3) in ('318'))  " +
                             //Added For DG kit KANBAN
                             " ORDER BY PartDesc";
                ...
                else
                {
                    strSql = " select BD.PartCode,partdesc+' [' +BD.partcode +'] ' as PartDesc,'' as SerialNo,u.UName,Qty,BD.Mob,P.Kit,p.conversionvalue,p.UomCode,'0' as StockQty,'0' as TotalQty," +
                             ...
                             " from BOMdetails BD Inner Join Part P On Bd.Partcode = P.partcode Inner join UOM U On P.UOMCode = U.Uid " +
                             " where bd.BOMCode ='" + ProdDts[0].Trim() + "'  " +
                             " and (BD.PartCode in (select Partcode from PartTOCDetailsSupplier where Product = 'KB' and ForPCCode = '01.004'))   " +
                             " and bd.KitCode = '" + strPrdPartCode.Trim() + "' " +
                             //Added For DG kit KANBAN
                             " Union All " +
                             " select BD.PartCode,partdesc + ' [' + BD.partcode + '] ' as PartDesc,'' as SerialNo,u.UName,Qty,BD.Mob,P.Kit,p.conversionvalue,p.UomCode,'0' as StockQty,'0' as TotalQty, " +
                             ...
                             " from BOMdetails BD Inner Join Part P On Bd.Partcode = P.partcode Inner join UOM U On P.UOMCode = U.Uid " +
                             " where bd.BOMCode ='" + ProdDts[0].Trim() + "' " +
                             " and(BD.PartCode in (select Partcode from PartTOCDetailsSupplier where Product = 'KB' and ForPCCode = '01.004' and Active='1'))   " +
                             " and bd.KitCode in (select Partcode From BOMDetails where BOMCode = '" + ProdDts[0].Trim() + "' and Partcode like '005%' and substring(partcode,10,3) in ('318'))  " +
                             //Added For DG kit KANBAN
                             " ORDER BY PartDesc";
        ...
        public string SubmitStage4(DGStage4Request Stage4Req)
        {
            ...
            if (Stage4Req.Remark == "Start")
            {
                ...
                if (dsDetailsSubV.Tables["tblPart"].Rows.Count > 0)
                {
                 ...
                 DGCFM = ComCon.getTranName("SELECT CFM FROM Part WHERE Partcode ='" + Stage4Req.ProductCode.Trim() + "' AND Active='1'", "processfeedback", "CFM", con, tran);
                ...
                try
                {
                    ...
                    sb.Append("'" + double.Parse(StrDGRate.Trim()) + "','" + CladdingRate.Trim() + "','" + Stage4Req.Remark.Trim() + "','" + ProdDts[0].Trim() + "','P')"); ;
                    cmd = new SqlCommand(sb.ToString(), con);
                    cmd.Transaction = tran;
                    cmd.ExecuteNonQuery();
                    cmd.Dispose();
                   ...
                   // string DGCFM = getName("SELECT CFM FROM Part WHERE Partcode ='" + PartCode[1].ToString().Trim() + "' AND Active='1' ", "PartCFM", "CFM");
                   


                    // Added by KB on 24/07/2024 for Paneltype is 0
                    ...
                    #region
                    strKanBan = "";
                    dsKanBan = ComCon.procTranDS("exec InternalTOCReq '" + Stage4Req.PCCode.Trim() + "' ", "tbl_RaiseReqDtsKanBan", con, tran);
                    if (dsKanBan != null && dsKanBan.Tables["tbl_RaiseReqDtsKanBan"].Rows.Count > 0)
                    {
                        ...
                        GetMaxValue = ComCon.GetMaxNo("MaterialRequisitionWithOutPlan", "REQ", Stage4Req.PCCode.Trim().Substring(0, 2), con, tran);
                        ...
                        sb.Append("'" + ComCon.yearEnd(con, tran) + "','" + Stage4Req.PCCode.Trim() + "','23.001','" + Stage4Req.ProductCode + "','" + Stage4Req.PCCode.Substring(0, 2) + "','1','P','WIP',");
                        sb.Append("'Auto Req For Plan No: " + Stage4Req.ProductCode + " and Prc No: " + PrcNo + "','1','1','1','KanBan')");
                        cmd = new SqlCommand(sb.ToString(), con);
                        cmd.Transaction = tran;
                        cmd.ExecuteNonQuery();
                        cmd.Dispose();
                    ...
                    //cmd.Parameters.AddWithValue("@EmpID", Stage4Req.EmpCode);
                    ...
                    //cmd.Parameters.AddWithValue("@CompanyCode", Stage4Req.PCCode.Substring(0, 2).Trim());
                    //cmd.Transaction = tran;
                    //cmd.ExecuteNonQuery();
                    //cmd.Dispose();
