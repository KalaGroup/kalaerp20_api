        ...
        public string Submit(JobCardRequest jobreq)
        {
            ...
            try
            {
              if (con.State == ConnectionState.Open) { con.Close(); } else { con.Open(); };
                ...
                JobCardNo = ComCon.GetMaxNo(jobreq.TableName, jobreq.Prefix, jobreq.CompCode, con, tran);

                SqlCommand cmd = new SqlCommand();
                sb.Remove(0, sb.Length);
                sb.Append("INSERT INTO JobCard(JobCode,Dt,Yr,MaxSrNo,PCCode,Remark,CompanyCode,Active,Auth)");
                sb.Append(" VALUES('" + JobCardNo.Trim() + "','" + DateTime.Now + "','" + ComCon.yearEnd(con,tran) + "','" + (jobreq.CompCode + strmax) + "',");
                sb.Append("'03.051','" + jobreq.Remark.Trim() + "','" + jobreq.CompCode + "','1','1')");
                cmd = new SqlCommand(sb.ToString(), con);
                cmd.Transaction = tran;
                cmd.ExecuteNonQuery();
                cmd.Dispose();

                int recCount =ComCon.CountChars(jobreq.JCDDts, ",");
                string[] strJCDDts = Regex.Split(jobreq.JCDDts, ",");
                int SrNo = 0;
